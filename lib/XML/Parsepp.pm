...
#~ regexp1_orig    = (?^umsx:[abc123])
#~ regexp2_orig    = (?^umsx:[^def456])
...
sub regexp_pattern {
    my ($fl, $pn) = $_[0] =~ m{\A \( \? ([\w\^\-]*) : (.*?) \) \z}xms
      or die "Error-0010: Internal Error - Can't disassemble quoted regexp = '$_[0]'";
    return ($pn, $fl);
}
...
sub negated {
    ...
    my ($caret, $class) =
      $pattern =~ m{\A \[ (\^?) (.*?) \] \z}xms
      or die "Error-0020: Internal Error - Can't parse regexp: $_[0] ==> (pattern = '$pattern', flags = '$flags')";
...
sub _more {
    ...
    if (length($buffer_text) > 100_000) {
        $self->crknum("Error-0030: Internal Error - Buffer overflow");
    }
    ...
    until ($buffer_breakout or $buffer_text eq '') {
        if ($buffer_action eq 'C') { # DEFACT: 'C' = character data
            if ($self->{_Stage} <= 2) {
                #~ my $fetch_regexp = $self->{_DocOpen} == 0 ? qr{[^<&]*}xms : qr{[^<&\]]*}xms;
                #~ my $limit_regexp = $self->{_DocOpen} == 0 ? qr{[<&]}xms   : qr{[<&\]]}xms;
                ...
                if ($buffer_text =~ m{\A (\s*) (\S) (.*) \z}xms) {
                    ...
                    $self->crknum("Error-0040: Internal Error - Can't parse buffer_text = '$buffer_text'");
                }
                ...
                if ($ch eq '') {
                    ...
                    $self->crknum("Error-0050: syntax error");
                }
                elsif ($ch =~ $rx_syn_syn) {
                    $self->_update_ctr($ch) if $level == 0;
                    $buffer_text    = $remainder;
                    ...
                    $self->crknum("Error-0060: syntax error");
                }
                else {
                    $self->crknum("Error-0070: not well-formed (invalid token)");
                }
            }
            else {
                #~ say {*STDERR} '';
                #~ say {*STDERR} "DEB-0010: buffer_text = '$buffer_text'";
                #~ say {*STDERR} "DEB-0020: regexp      = $limit_regexp";

                if ($buffer_text =~ m{\A ([^<&]*) ([<&]) (.*) \z}xms) {
                    my ($emit, $ch, $remainder) = ($1, $2, $3);

                    #~ say {*STDERR} "DEB-0030: emit = '$emit', ch = '$ch', remainder = '$remainder'";
                ...
                #~ say {*STDERR} "DEB-0040: buffer_text = '$buffer_text'";
            ...
            if ($self->{_QChar} eq q{'}) {
                ...
                $self->crknum("Error-0080: Internal Error - invalid QChar = '".$self->{_QChar}."'");
            }
            ...
            if ($ch eq '') {
                ...
                $self->crknum("Error-0090: not well-formed (invalid token)");
            }
        }
            ...
            if ($emit =~ m{($ng_syn_syn)}xms) {
                ...
                if ($out =~ $rx_tok_syn or $out eq '>') {
                    $self->crknum("Error-0100: syntax error");
                }
                else {
                    $self->crknum("Error-0110: not well-formed (invalid token)");
                }
            }
            else {
                if ($ch eq '') {
                    ...
                    $self->crknum("Error-0120: syntax error");
                }
            }
        }
            ...
            if ($buffer_text =~ m{\A . ([^<;]*) ([<;]) (.*) \z}xms) {
                my ($emit, $ch, $remainder) = ($1, $2, $3);
                unless ($ch eq ';') {
                    $self->crknum("Error-0130: not well-formed (invalid token)");
                }
            ...
            #~ say {*STDERR} "DEB-0005: c1 = '$c1', c2 = '$c2', c3 = '$c3'";

            if ($c2 eq '!' and $c3 eq '-') {
                $buffer_action = '!'; # DEFACT: '<!-- ... -->' a comment

                #~ say {*STDERR} "DEB-0020:    --> Comment, action = '$buffer_action'";
            ...
            if ($c2 eq '!' and $c3 eq '[') {
                ...
                #~ say {*STDERR} "DEB-0030:    --> CDATA, action = '$buffer_action'";
            ...
            if ($c2 eq '!' and $c3 =~ m{\w}xms) {
                ...
                #~ say {*STDERR} "DEB-0030:    --> DocType, action = '$buffer_action'";
            ...
            unless ($prefix eq '<!--') {
                $self->crknum("Error-0150: not well-formed (invalid token)");
            }
            if ($buffer_text =~ m{\A (.*? -->) (.*) \z}xms) {
        ...
        # pour identifier les différents possibilités de DTD (DOCTYPE, ELEMENT, ATTLIST, etc...), voir: http://www.u-picardie.fr/~ferment/xml/xml02.html
            ...
            if ($finpos != -1) {
                ...
                unless ($terminal eq '>' or $terminal eq '[') {
                    $self->crknum("Error-0170: Internal Error - found terminal char ('$terminal') not equal to ('>', '[')");
                }
            ...
            unless ($self->{_DocOpen}) {
                $self->crknum("Error-0180: Internal Error - Can't close a closed Doctype");
            }
            ...
            #~ say {*STDERR} '';
            #~ say {*STDERR} "DEB-0010: buffer_text = '$buffer_text'";
            #~ say {*STDERR} "DEB-0012:                0....+....1....+....2....+....3....+....4....+....5....+....6....+....7";

            my $finpos = -1;
            pos($buffer_text) = 0;
            while ($buffer_text =~ m{\G \s* (?: ([^'"\s]+) | ' [^']* ' | " [^"]* " ) }xmsgc) {

                #~ say {*STDERR} "DEB-0020: \$1 = '", ($1 // '*undef*'), "'";

                if (defined $1) {
                    my $mp = $-[1];

                    #~ say {*STDERR} "DEB-0030: mp = $mp";

                    my $fragment = $1;

                    #~ say {*STDERR} "DEB-0032: fragment = '$fragment'";
                    #~ say {*STDERR} "DEB-0034:             0....+....1....+....2....+....3....+....4....+....5....+....6....+....7";

                    if ($fragment =~ m{>}xms) {
                        $finpos = $mp + $-[0];

                        #~ say {*STDERR} "DEB-0040: \$-[0] = $-[0], finpos = $finpos";
            ...
            $self->crknum("Error-0200: Internal Error - invalid buffer_action = '$buffer_action'");
        }
    }
...
sub _emit_Amp {
    ...
    my ($var) = $ampersand =~ m{\A & ([^&;]+) ; \z}xms
      or $self->crknum("Error-0210: Internal Error - Can't parse ampersand = '$ampersand'");
    ...
    if ($var =~ m{\A \# (\d+) \z}xms) {
        ...
        unless (defined $rhs) {
            if ($level == 0) {
                $self->crknum("Error-0220: undefined entity");
            }
            else {
                $self->crknum("Error-0230: error in processing external entity reference");
            }
        }
        ...
        if ($code eq 'L') {
            ...
            if ($self->{_Seen}{$var}) {
                $self->crknum("Error-0240: error in processing external entity reference");
            }
            $self->{_Seen}{$var} = 1;
            ...
            if ($cb_Exen) {
                ...
                unless ($self->{_Text} eq '') {
                    $self->crknum("Error-0250: error in processing external entity reference");
                }
                if (@{$self->{_Stack}}) {
                ...
                open my $ifh, '<', $value
                  or $self->crknum("Error-0270: Handler couldn't resolve external entity\n"."404 File `$filepath' does not exist");
            ...
            if ($self->{_Seen}{$var}) {
                $self->crknum("Error-0300: recursive entity reference");
            }
            $self->{_Seen}{$var} = 1;
            ...
            unless ($self->{_Text} eq '') {
                $self->crknum("Error-0310: unclosed token");
            }
            if (@{$self->{_Stack}}) {
                $self->crknum("Error-0320: asynchronous entity");
            }

            $self->{_Seen}{$var} = 0;
        }
        else {
            $self->crknum("Error-0330: Internal Error - Found invalid code '$code' not equal to ('F', 'L', 'T')");
        }
    }
}
...
sub _emit_Char {
    ...
    unless ($self->{_Stage} == 3) {
        ...
        if ($emit =~ m{\S}xms) {
            if ($self->{_Stage} == 4) {
                $self->crknum("Error-0340: junk after document element");
            }
            else {
                $self->crknum("Error-0350: Internal Error - non-space data");
            }
        }
    }
    ...
    if ($default) {
        ...
        while ($emit =~ m{\G (?: ([^\n]+) | ([\n]) ) }xmsgc) {
            ...
            if (defined $1) {
                ...
                $self->crknum("Error-0360: Internal Error - inconsistent result from regexp");
            }
        ...
        unless ($emit =~ m{\G (.*) \z}xms) {
            $self->crknum("Error-0370: Internal Error - Can't find regexp rest in CHAR");
        }
        my $rest = $1;
        if ($rest ne '') {
            $self->crknum("Error-0380: Internal Error - Invalid rest ($rest) in CHAR regexp");
        }
    }
}
...
sub _emit_Start {
    ...
    my ($elem, $param, $term) = $emit =~ m{\A < \s* ([,\-.\w:\[|]+) (.*?) (/?) > \z}xms
      or $self->crknum("Error-0390: Internal Error - Can't decompose start = '$emit'");
    ...
    while ($param =~ m{\G \s* ([,\-.\w:\[|]+) \s* = \s* (?: ' ([^']*) ' | " ([^"]*) " ) }xmsgc) {
        ...
        if (defined $2) {
            ...
            $self->crknum("Error-0400: Internal Error - Can't match any param");
        }

        if ($def_txt =~ m{<}xms) {
            $self->crknum("Error-0410: not well-formed (invalid token)");
        }

        $def_txt =~ s{\n}' 'xmsg;

        #~ say {*STDERR} "DEB-0005: def_txt = '$def_txt'";

        my $def_res = '';
        #~ say {*STDERR} "DEB-0010:   def_res = ''";
        ...
        while ($def_txt =~ m{\G ([^&]*) & ([^&;]+) ; }xmsgc) {
            ...
            #~ say {*STDERR} "DEB-0020:   def_res .= (\$1)   '$1' --> '$def_res', var = '$var'";

            my $rhs = $self->{_Var}{$var};

            unless (defined $rhs) {
                $self->crknum("Error-0420: undefined entity");
            }
            ...
            unless ($code eq 'L') {
                $self->crknum("Error-0430: reference to external entity in attribute");
            }

            $def_res .= $value;
            #~ say {*STDERR} "DEB-0030:   def_res .= (val)  '$value' --> '$def_res'";
        }
        unless ($def_txt =~ m{\G (.*) \z}xms) {
            $self->crknum("Error-0440: Internal Error - Can't find regexp rest in ELEMENT");
        }

        my $rest = $1;
        if ($rest =~ m{&}xms) {
            $self->crknum("Error-0450: not well-formed (invalid token)");
        }

        $def_res .= $rest;
        #~ say {*STDERR} "DEB-0040:   def_res .= (rest) '$rest' --> '$def_res'";

        push @attr, $def_var, $def_res;
    }
    unless ($param =~ m{\G (.*) \z}xms) {
        $self->crknum("Error-0460: Internal Error - Can't find regexp rest in START");
    }

    my $rest = $1;
    if ($rest =~ m{\S}xms) {
        if ($level == 0) {
            $self->crknum("Error-0470: not well-formed (invalid token)");
        }
        else {
            $self->crknum("Error-0480: error in processing external entity reference");
        }
    }
    ...
    if ($term eq '/') {
        if ($self->{_Scount} < 1) {
            $self->crknum("Error-0490: Internal Error - Underflow in Scount");
        }
        $self->{_Scount}--;

        my $ele_from_stack = pop @$bstack;
        unless (defined $ele_from_stack) {
            $self->crknum("Error-0500: Internal Error - Underflow in stack");
        }

        unless ($elem eq $ele_from_stack) {
            $self->crknum("Error-0510: Internal Error - Mismatch of Start- and End-tag, start = '$ele_from_stack', end = '$elem'");
        }
        ...
        $self->crknum("Error-0520: Internal Error - in START found closing tag '$term'");
    }
}
...
sub _emit_End {
    ...
    my ($elem) = $emit =~ m{\A < \s* / \s* ([,\-.\w:\[|]+) \s* > \z}xms
      or $self->crknum("Error-0530: not well-formed (invalid token)");

    if ($self->{_Scount} < 1) {
        $self->crknum("Error-0540: not well-formed (invalid token)");
    }
    $self->{_Scount}--;
    ...
    unless (defined $ele_from_stack) {
        if ($hist =~ m{F}xms) {
            $self->crknum("Error-0550: error in processing external entity reference");
        }
        else {
            $self->crknum("Error-0560: asynchronous entity");
        }
    }

    unless ($elem eq $ele_from_stack) {
        $self->crknum("Error-0570: mismatched tag");
    }
...
sub _emit_Proc {
    ...
    my ($target, $data) = $emit =~ m{\A <\? ([,\-.\w:\[|]+) \s* (.*) \?> \z}xms
      or $self->crknum("Error-0580: not well-formed (invalid token)");

    if ($target =~ m{\A xml}xmsi) {
        unless ($self->{_ItemCount} == 0) {
            $self->crknum("Error-0590: XML or text declaration not at start of entity");
        }

        my @attr;
        pos($data) = 0;

        #~ say {*STDERR} "pos = ", pos($data), ", data = '$data'";

        while ($data =~ m{\G \s* ([,\-.\w:\[|]+) \s* = \s* (?: ' ([^']*) ' | " ([^"]*) " ) }xmsgc) {
            #~ say {*STDERR} "  pos = ", pos($data), ", inside loop \$1 = '$1', \$2 = ", ($2 // 'undef'), ", \$3 = ", ($3 // 'undef'), ".";
            ...
            if (defined $2) {
                ...
                $self->crknum("Error-0600: Internal Error - Can't match any param");
            }
        }

        #~ say {*STDERR} "pos = ", pos($data), ", after loop";

        unless ($data =~ m{\G (.*) \z}xms) {
            $self->crknum("Error-0610: Internal Error - Can't find regexp rest in PROC");
        }

        my $rest = $1;
        if ($rest =~ m{\S}xms) {
            $self->crknum("Error-0620: XML declaration not well-formed");
        }
        ...
        for my $at (@attr) {
            if ($at->[0] eq 'version') {
                if (defined $ver) {
                    $self->crknum("Error-0630: XML declaration not well-formed");
                }
                $ver = $at->[1];
            }
            elsif ($at->[0] eq 'encoding') {
                if (defined $enc) {
                    $self->crknum("Error-0640: XML declaration not well-formed");
                }
                $enc = $at->[1];
            }
...
sub _emit_Comment {
    ...
    my ($comment) = $emit =~ m{\A <!-- (.*?) --> \z}xms or
      $self->crknum("Error-0690: Internal Error - Can't decompose comment '$emit'");
...
sub _emit_CloseDoc {
    ...
    $emit =~ m{\A \] \s* > \z}xms
      or $self->crknum("Error-0700: Internal Error - Invalid closedoc: '$emit'");

    unless ($self->{_DocOpen}) {
        $self->crknum("Error-0710: Internal Error - closedoc found without DocOpen");
    }
...
sub _emit_Dtd {
    ...
    if ($self->{_Stage} > 2) {
        $self->crknum("Error-0720: not well-formed (invalid token)");
    }

    my ($type, $data, $term) = $emit =~ m{\A <! (\w+) \s+ (\S .*) ([\[>]) \z}xms
      or $self->crknum("Error-0730: not well-formed (invalid token)");
    ...
    while ($data =~ m{\G \s* (?: ([^'"\(\s]+) | ' ([^']*) ' | " ([^"]*) " | \( ([^\)]*) \) ) }xmsgc) {
        if (defined $1) {
            ...
            $self->crknum("Error-0740: Internal Error - regexp undefined");
        }
    }

    unless ($data =~ m{\G (.*) \z}xms) {
        $self->crknum("Error-0750: Internal Error - Can't find regexp rest");
    }

    my $rest = $1;
    if ($rest =~ m{\S}xms) {
        $self->crknum("Error-0760: syntax error");
    }
    ...
    if ($type eq 'DOCTYPE') {
        ...
        $self->crknum("Error-0770: syntax error");
    }

    unless ($type eq 'DOCTYPE' or $term eq '>') {
        $self->crknum("Error-0780: syntax error");
    }
}
...
sub _parse_Doctype {
    ...
    unless (defined $param0) {
        $self->crknum("Error-0800: Internal Error - Not enough elements in DOCTYPE");
    }
    ...
    if (defined $param1) {
        ...
        if ($param1->[1] eq 'SYSTEM') {
            ...
            $self->crknum("Error-0830: syntax error");
        }
    ...
    if (defined $public) {
        if ($public =~ m{[\]\[\\]}xms) {
            $self->crknum("Error-0870: illegal character(s) in public id");
        }
    }
    ...
    if ($self->{_DocOpen}) {
        $self->crknum("Error-0890: Internal Error - DOC is open");
    }
...
sub _parse_Entity {
    ...
    #~ {
        #~ local $" = " => ";
        #~ print {*STDERR} "DEB-0010: parse Entity, plist = ";
    ...
    unless (defined $param0) {
        $self->crknum("Error-0900: Internal Error - Not enough elements in ENTITY");
    }
    ...
    unless (defined $param1) {
        $self->crknum("Error-0920: syntax error");
    }
    ...
    if ($param1->[0] eq 'Q') {
        ...
        unless ($param1->[1] eq 'SYSTEM') {
            if ($param1->[1] eq 'PUBLIC') {
                $self->crknum("Error-0930: syntax error");
            }
            else {            
                $self->crknum("Error-0940: not well-formed (invalid token)");
            }
        }
        ...
        if (defined $param3) {
            unless ($param3->[0] eq 'B') {
                $self->crknum("Error-0970: syntax error");
            }
...
sub _parse_Element {
    ...
    unless (defined $param0) {
        $self->crknum("Error-1030: Internal Error - Not enough elements in ELEMENT");
    }
    ...
    unless ($param1->[0] eq 'P') {
        $self->crknum("Error-1060: syntax error");
    }
...
sub _parse_Attlist {
    ...
    unless (defined $param0) {
        $self->crknum("Error-1090: Internal Error - Not enough elements in ATTLIST");
    }
    ...
    while (@$plist) {
        ...
        unless (defined $param1) {
            $self->crknum("Error-1110: Internal Error - Not enough elements in ATTLIST-PARAM1");
        }

        unless ($param1->[0] eq 'B') {
            $self->crknum("Error-1120: syntax error");
        }
        ...
        if ($param2->[0] eq 'B' and $param2->[1] eq 'NOTATION') {
            ...
            unless (defined $pm2b) {
                $self->crknum("Error-1140: syntax error");
            }
...
sub _parse_Notation {
    ...
    unless (defined $param0) {
        $self->crknum("Error-1220: Internal Error - Not enough elements in NOTATION");
    }
    ...
    if (defined $param1) {
        ...
        if ($param1->[1] eq 'SYSTEM') {
            ...
            $self->crknum("Error-1250: syntax error");
        }
...
sub _plausi {
    ...
    my $tp = $pl eq 'D' || $pl eq 'F' || $pl eq 'U' || $pl eq 'I' || $pl eq 'L' || $pl eq 'O' || $pl eq 'T' ? 'DTD' 
           ...
           : $pl eq 'P'                                                                                     ? 'PRC'
           : $pl eq 'X'                                                                                     ? 'XML'
           : $self->crknum("Error-1310: Internal Error - encountered plausi code = '$pl'");

    my $stage = $self->{_Stage};

    #~ say {*STDERR} "DEB-0010: stage before = $stage, pl = '$pl', tp = '$tp'";
    ...
    if ($stage == 1) {
        unless ($pl eq 'X' or $pl eq 'C' or $pl eq '!') {
            $self->crknum("Error-1320: Internal Error - Found invalid callback, plausi = '$pl' at stage 1");
        }
        if ($pl eq 'X') {
            $stage = 2;
        }
        ...
        unless ($tp eq 'DTD' or $pl eq 'C' or $pl eq '!') {
            $self->crknum("Error-1330: Internal Error - Expected 'DTD', but found '$tp', plausi = '$pl' at stage 2");
        }
    }
    elsif ($stage == 3) {
        unless ($tp eq 'TAG' or $tp eq 'PRC' or $tp eq 'TXT') {
            $self->crknum("Error-1340: Internal Error - Expected 'TAG', 'PRC' or 'TXT', but found '$tp', plausi = '$pl' at stage 3");
        }
        ...
        unless ($pl eq 'C') {
            $self->crknum("Error-1350: junk after document element");
        }
    }
    else {
        $self->crknum("Error-1360: Internal Error - invalid stage = $stage");
    }

    #~ say {*STDERR} "DEB-0020: stage after  = $stage";
...
sub parse_done {
    ...
    if ($self->{_Action} eq 'F') {
        $self->crknum("Error-1370: unclosed token");
    }

    if ($self->{_Action} eq 'G') {
        $self->crknum("Error-1380: syntax error");
    }

    if (@{$self->{_Stack}}) {
        $self->crknum("Error-1390: no element found");
    }

    unless ($self->{_Scount} == 0) {
        $self->crknum("Error-1400: Internal Error -  no element found");
    }

    unless ($self->{_Text} eq '') {
        $self->crknum("Error-1410: unclosed token");
    }
...
sub crknum {
    ...
    # if ($err =~ m{at \s+ line \s+ (\d+), \s+ column \s+ (\d+), \s+ byte \s+ (\d+) \s+ at \s+ (\S+) \s+ line \s+ (\d+) \s* \z}xms) {
